...
describe("Reducer", () => {
  ...
  test("Debe renderizar una cita", () => {
    ...
    const result = citaReducer(initialState, {
  ...
  test("estado del error", () => {
    const result = citaReducer(initialState, {
      type: obtenerCitaAsync.rejected,
    });
    expect(result.estado).toEqual(ESTADO_FETCH.ERROR);
  });
  test("estado en cargando", () => {
    const result = citaReducer(initialState, {
      type: obtenerCitaAsync.pending,
    });
    expect(result.estado).toEqual(ESTADO_FETCH.CARGANDO);
  });

  test("estado en inactivo", () => {
    const result = citaReducer(initialState, {
      type: obtenerCitaAsync.fulfilled,
      payload: null,
    });
    expect(result.estado).toEqual(ESTADO_FETCH.INACTIVO);
  });
